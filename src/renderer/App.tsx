...
function Index() {
  ...
  const [packageSize,setPackageSize] = useState({value:0,label:'10M',size:"10000000"});
  ...
  let packageSizeList = [{value:0,label:'10M',size:"10000000"},{value:1,label:'40M',size:"40000000"},{value:2,label:'100M',size:"100000000"}] // mac 1mb == 1000kb
  if(!isMac()){
    packageSizeList = [{value:0,label:'10M',size:"10485760"},{value:1,label:'40M',size:"41943040"},{value:2,label:'100M',size:"104857600"}] // win 1mb == 1024kb
  }
    ...
    onChange(info) {
      ...
      if(fileList && fileList.length){
        ...
        if(fileList.length > 1){
          ...
          pageData.fileName = "thearchive." + fileNameArr[fileNameArr.length - 1];
        }
        let tempFileList = fileList.map((item)=>{
          return {path:item.originFileObj?.path, fileName:item.originFileObj?.webkitRelativePath || item.originFileObj?.name}
        })
  ...
  return (
    <>
      <Dragger {...props}>
        ...
        <p className="ant-upload-text">点击或者拖拽文件到选框中</p>
        <p className="ant-upload-hint">
        </p>
      </Dragger>

      <Modal title="压缩" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
      ...
      <Form.Item label="是否分卷">
        <Switch defaultChecked = {isPackage} onChange={switchOnChange} />
      </Form.Item>

      <Form.Item label="大小" style={{'display': isPackage?'block':'none'}}>
